...
export default function SettingsPage() {
  ...
  const repoCardRefs = useRef<Record<string, HTMLDivElement | null>>({})
  ...
  const handleRepoScreenshot = async (repoFullName: string) => {
    const repoCard = repoCardRefs.current[repoFullName]
    if (!repoCard || downloadingRepos[repoFullName]) return

    setDownloadingRepos((current) => ({ ...current, [repoFullName]: true }))
    try {
      await downloadAsImage(repoCard, `repo-${repoFullName.replace('/', '-')}.png`)
    } finally {
          ...
          <section className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
            ...
            <Card className="border-white/60 bg-slate-950 p-6 text-white shadow-lg shadow-slate-950/15">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-300">
                We search GitHub for pull requests you authored that were merged, then refresh your cached repository stats.
              </p>
                  ...
                  {repos.map((repo) => {
                    ...
                    return (
                      <div
                        ...
                        ref={(element) => {
                          repoCardRefs.current[repo.repo_full_name] = element
                        }}
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                            ...
                            <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 ${
                              ...
                              {repo.is_active ? 'Showing publicly' : 'Cached only'}
                            </span>
                          </div>

                          <div className="flex flex-col gap-2 rounded-2xl border border-border/70 bg-card/70 px-3.5 py-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="min-w-0">
                              <p className="text-sm font-medium text-foreground">Show on profile, timeline, and feed</p>
                              <p className="text-xs text-muted-foreground">
                                Turn this repo on to make only this repository&apos;s merged PRs public.
                              </p>
                            </div>
                            ...
                            >
                              ...
                              {isDownloading ? 'Downloading…' : 'Screenshot'}
                            </Button>
                          </div>
                        </div>
                      </div>
