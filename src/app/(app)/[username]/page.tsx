...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  // If profile not found, trigger 404
  ...
  // Fetch PRs for this user, ordered by merged_at DESC
  ...
  // Transform PRs to include profile data
  ...
  // Get current viewer for follow button rendering
          ...
          <CardContent className="pt-6">
            <div className="flex items-start gap-6">
              ...
              <div className="flex items-center gap-2">
                {/* Follow button — only for authenticated viewers on other profiles */}
                {currentUser && !isOwnProfile && (
                  <FollowButton
...
export async function generateMetadata({ params }: ProfilePageProps) {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('display_name, github_username, github_avatar_url')
    .eq('github_username', username)
    .single()
  ...
  return {
    title: `${displayName} (@${typedProfile.github_username}) - PR Portfolio`,
    description: `View ${displayName}'s pull request portfolio on mypr.pro.bd`,
    openGraph: {
      title: `${displayName} - PR Portfolio`,
      description: `View ${displayName}'s pull request portfolio on mypr.pro.bd`,
      images: typedProfile.github_avatar_url ? [typedProfile.github_avatar_url] : []
    }
  }
}
